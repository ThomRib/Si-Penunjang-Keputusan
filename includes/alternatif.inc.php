...
class Alternatif {
	...
	function insert() {
		...
		$stmt->bindParam(11, $this->no_telp);
		$stmt->bindParam(12, $this->pendidikan);
	...
	function readByFilter() {
		...
		$query = "SELECT * FROM data_alternatif JOIN nilai_awal ON data_alternatif.id_alternatif = nilai_awal.id_alternatif WHERE nilai_awal.keterangan = 'B'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
