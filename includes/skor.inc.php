...
class Skor {
	...
	function readAll4() {
		...
		return $stmt;
		// $query = $conn->query("SELECT * FROM data_alternatif");
	}
	...
	function readSum5($a, $b) {
		$query = "SELECT sum(hasil_analisa_alternatif) AS jumkr3 FROM {$this->table_name} WHERE alternatif_pertama='$a' AND id_kriteria='$b'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['jumkr3'];
